...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.M2000CSimple.Functions
{
    ...
    public class TestValues : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[4000];
    ...
    private static BindingValue _xValue = new BindingValue(1);
        ...
        public TestValues(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            int compteur;
            for (compteur = 0; compteur < 4000; compteur++)
            {
                _dataElements.SetValue(new DCSDataElement(compteur.ToString() , "%.4f", false), compteur);
            }
